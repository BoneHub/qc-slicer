...
class BoneHubQualityCheckWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def downloadAndLoad(self, handout):
        ...
        def fetch():
            ...
            return imagePath, segmentationPath

        try:
            imagePath, segmentationPath = self.runWithProgress(
                _("Downloading {key}...").format(key=handout.get("subject_key", "")), fetch
            )
        except QCClientError as error:
        ...
        if imagePath is None:
            slicer.util.errorDisplay(
                _("The server has no image for {key}, so it cannot be reviewed. Release it and ask for the next one.").format(
                    key=handout.get("subject_key", "")
                ),
                windowTitle=_("No image"),
            )
            self.updateGuiFromSession()
            return
        ...
        try:
            ...
            self.logic.loadSubject(handout, imagePath, segmentationPath)
    ...
    def updateGuiFromSession(self):
        ...
        self.ui.reviewCollapsibleButton.enabled = holding
        ...
        if holding:
            ...
            self.ui.subjectKeyLabel.text = _("{key}  (dataset {dataset}, subject {subject})").format(
                key=handout.get("subject_key", ""),
...
class BoneHubQualityCheckLogic(ScriptedLoadableModuleLogic):
    ...
    def clearScene(self):
        ...
        for node in (self.segmentationNode, self.imageVolumeNode):
            if node is not None and slicer.mrmlScene.IsNodePresent(node):
                slicer.mrmlScene.RemoveNode(node)
        self.imageVolumeNode = None
        self.segmentationNode = None

    def loadSubject(self, handout, imagePath, segmentationPath=None):
        """Load one subject's image and its BoneHub segmentation, one segment per label."""
        self.clearScene()
        ...
        if segmentationPath is not None:
            ...
            self.segmentationNode = slicer.mrmlScene.AddNewNodeByClass(
                "vtkMRMLSegmentationNode", f"{subjectKey}_segmentation"
            )
            self.segmentationNode.CreateDefaultDisplayNodes()

        self.segmentationNode.SetReferenceImageGeometryParameterFromVolumeNode(self.imageVolumeNode)
        ...
        slicer.util.setSliceViewerLayers(background=self.imageVolumeNode, fit=True)
    ...
    def exportReviewedSegmentation(self, path):
        """Write the segmentation as a BoneHub ``.seg.nrrd`` on the image's voxel grid.
        ...
        """
        if self.segmentationNode is None or self.imageVolumeNode is None:
            raise RuntimeError("No subject is loaded.")
        ...
        segmentation = self.segmentationNode.GetSegmentation()
        shape = slicer.util.arrayFromVolume(self.imageVolumeNode).shape
        ...
        for segmentId in byValue:
            ...
            mask = slicer.util.arrayFromSegmentBinaryLabelmap(self.segmentationNode, segmentId, self.imageVolumeNode)
            if mask is None or mask.size == 0:
                continue
            if mask.shape != shape:
                raise RuntimeError(f"Segment '{name}' could not be brought onto the image's voxel grid.")
        ...
        self.imageVolumeNode.GetIJKToRASMatrix(ijkToRas)
    ...
    def openSegmentEditor(self):
        ...
        if hasattr(editor, "setSourceVolumeNode"):
            editor.setSourceVolumeNode(self.imageVolumeNode)
        else:  # Slicer 5.0 and older
            editor.setMasterVolumeNode(self.imageVolumeNode)
...
class BoneHubQualityCheckTest(ScriptedLoadableModuleTest):
    ...
    It exercises the parts that do not need a server: the label map, and the round trip of
    a segmentation through the scene -- loaded from a BoneHub ``.seg.nrrd``, edited, and
    written back -- which is where the dataset could silently be corrupted.
    """
    ...
    def runTest(self):
        ...
        self.test_ASegmentationCanBeStartedFromScratch()
    ...
    def test_ASegmentationCanBeStartedFromScratch(self):
        ...
        segmentId = logic.addEmptySegment("FEMUR_LEFT")
        with self.assertRaises(RuntimeError):
            logic.addEmptySegment("FEMUR_LEFT")
        with self.assertRaises(RuntimeError):
            logic.addEmptySegment("NOT_A_BONE")
        painted = (expected == 710000001).astype(np.uint8)
        slicer.util.updateSegmentBinaryLabelmapFromArray(painted, logic.segmentationNode, segmentId, volumeNode)

        path = Path(tempfile.mkdtemp()) / f"reviewed{SEGMENTATION_SUFFIX}"
        self.assertEqual(logic.exportReviewedSegmentation(path), {"FEMUR_LEFT": 710000001})
        image, values, _segments = self._readBoneHubFile(path)
        self.assertTrue(np.array_equal(values, np.where(painted > 0, 710000001, 0)))
        self._assertSameGridAsTheImage(image, self.imagePath)
    ...
    # ---------------------------------------------------------------- helpers
    def _buildSubject(self, withSegmentation=True, names=None):
        """A small subject in the scene: an image and a BoneHub segmentation of two labels.

        The image is oblique, so a mix-up between Slicer's RAS and the files' LPS, or of the
        axis order, shows. ``names`` renames segments by number in the file, leaving their
        BoneHubValue tags as they are.
        """
        ...
        logic.loadSubject({"subject_key": "001_000001"}, imagePath, segmentationPath)
        self.imagePath = imagePath
        return logic, logic.imageVolumeNode, expected
